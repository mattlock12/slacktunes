...
class YoutubeService(ServiceBase):
    ...
    def get_track_info_from_link(self, link):
        ...
        resp = service.videos().list(part='snippet', id=video_id)
    ...
    def fuzzy_search_for_track(self, track_info):
        ...
        try:
            search_results = service.search().list(q=track_info.get_track_name(), **search_kwargs).execute()
        except Exception as e:
            # TODO: better exception handling
            return None
...
class SpotifyService(ServiceBase):
    ...
    def get_wrapped_service(self):
        ...
        service = Spotipy(client_credentials_manager=credentials_manager)
        self.service = service
        return service
    ...
    def get_track_info_from_link(self, link):
        ...
        service = self.get_wrapped_service()
        try:
            track_info = service.track(track_id=track_id)['tracks'][0]
        except Exception as e:
    ...
    def fuzzy_search_for_track(self, track_info):
        ...
        results = service.search(q=search_string, **search_kwargs)
        if not results:
            return None
